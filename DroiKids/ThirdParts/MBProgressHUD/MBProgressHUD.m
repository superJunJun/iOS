...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        CGSize dims;
#ifdef App_iOS6Supported
        if([NSString resolveInstanceMethod:@selector(sizeWithAttributes:)])
        {
        ...
        if (nil != self.detailsLabelText) {
            ...
            if([NSString resolveInstanceMethod:@selector(sizeWithAttributes:)])
            {
                labelSize = [detailsLabel.text boundingRectWithSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:detailsLabel.font} context:nil].size;
            }
            else
            {
                labelSize = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) lineBreakMode:detailsLabel.lineBreakMode];
            }
#else
            labelSize = [detailsLabel.text boundingRectWithSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:detailsLabel.font} context:nil].size;
#endif//App_iOS6Supported
